...
(function () {
  ...
  E.stopBoard = (asOf, basis) => {
    ...
    const out = [];
    live.forEach(d => { const m = E.metrics(d, asOf, { basis }); m.perSku.forEach(p => { const st = p.alloc == null ? 'no allocation' : p.alloc === 0 ? (m.known && p.units > 0 ? 'STOP now' : 'not enrolled') : p.units >= p.stop ? 'STOP now' : p.units >= p.alloc * D.settings.stop_warn ? 'near limit' : p.proj >= p.stop ? 'will breach' : 'ok'; out.push({ deal: d.id, tag: d.tag, type: d.type, ...p, status: st }); }); });
    return out;
  };
  ...
  E.stockRisk = (m, mult) => {
    ...
    return m.perSku.map(p => {
      ...
      else if (p.pctStop != null && p.pctStop >= s.stop_line) { flag = 'REMOVE FROM DEAL'; cls = 'bad'; why = p.alloc <= 5 ? `sold ${p.units} against a safe allocation of only ${p.alloc}` : `sold ${Math.round(p.pctStop * 100)}% of the stop line`; }
